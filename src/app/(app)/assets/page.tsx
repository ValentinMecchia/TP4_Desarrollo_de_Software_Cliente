  ...
  return (
    <div className="container mx-auto py-8 px-4 sm:px-6 lg:px-8">
      ...
      <Card className={`mb-8 w-full ${cardHoverEffect}`}>
        ...
        <CardContent>
          {/* The Table component from shadcn/ui already has overflow-auto for responsiveness */}
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="font-semibold text-muted-foreground/90 whitespace-nowrap">Asset Name</TableHead>
                <TableHead className="font-semibold text-muted-foreground/90 whitespace-nowrap">Symbol</TableHead>
                <TableHead className="text-right font-semibold text-muted-foreground/90 whitespace-nowrap">Price</TableHead>
                <TableHead className="text-right font-semibold text-muted-foreground/90 whitespace-nowrap">Change (24h)</TableHead>
                <TableHead className="text-right font-semibold text-muted-foreground/90 whitespace-nowrap">Market Cap</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {assets.map((asset) => (
                <TableRow key={asset.id} className="transition-colors duration-150 hover:bg-muted/50">
                  <TableCell className="font-medium py-3.5 whitespace-nowrap">{asset.name}</TableCell>
                  <TableCell className="py-3.5 whitespace-nowrap">{asset.symbol}</TableCell>
                  <TableCell className="text-right py-3.5 whitespace-nowrap">${asset.price.toLocaleString()}</TableCell>
                  <TableCell className={`text-right font-medium py-3.5 whitespace-nowrap ${asset.change >= 0 ? 'text-positive' : 'text-negative'}`}>
                    <span className="inline-flex items-center">
                      {asset.change >= 0 ? <ArrowUpRight className="h-4 w-4 mr-1" /> : <ArrowDownRight className="h-4 w-4 mr-1" />}
                      {asset.changePercent.toFixed(2)}%
                    </span>
                  </TableCell>
                  <TableCell className="text-right py-3.5 whitespace-nowrap">{asset.marketCap}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
          ...
          <ChartContainer config={chartConfig} className="h-[400px] w-full aspect-video">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 20, right: 20, bottom: 5, left: 0 }}>
                ...
                <Bar dataKey="TII" fill="var(--color-TII)" radius={[4, 4, 0, 0]} isAnimationActive={true} animationDuration={800} />
                <Bar dataKey="EES" fill="var(--color-EES)" radius={[4, 4, 0, 0]} isAnimationActive={true} animationDuration={800} animationBegin={200} />
                <Bar dataKey="GHC" fill="var(--color-GHC)" radius={[4, 4, 0, 0]} isAnimationActive={true} animationDuration={800} animationBegin={400} />
              </BarChart>
            </ResponsiveContainer>
          </ChartContainer>
