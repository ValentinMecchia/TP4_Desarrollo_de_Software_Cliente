      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {news.map((article) => (
          <Card key={article.id} className={`flex flex-col ${cardHoverEffect}`}>
            <CardHeader>
              {article.imageUrl && (
                 <Image src={article.imageUrl} alt={article.title} width={400} height={200} className="w-full h-48 object-cover rounded-t-lg mb-4" data-ai-hint="news business"/>
              )}
            ...
            <CardContent className="flex-grow">
              ...
              {insights[article.id] && (
                <div className="mt-4 p-3 bg-accent/10 dark:bg-accent/20 rounded-lg border border-accent/30">
                  <h4 className="text-sm font-semibold text-accent flex items-center"><Zap className="w-4 h-4 mr-2 text-accent" />AI Insight</h4>
                  <p className="text-xs mt-1">
                    <strong>Relevance:</strong> 
                    <span className={`font-medium ml-1 ${
                      insights[article.id]?.relevance === 'high' ? 'text-positive' :
                      insights[article.id]?.relevance === 'medium' ? 'text-yellow-500 dark:text-yellow-400' : // Using yellow for medium
                      'text-negative'
